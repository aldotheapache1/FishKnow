...
import com.FishKnow.arcoremeasure.renderer.RectanglePolygonRenderer;
...
public class ArMeasureActivityAlt extends AppCompatActivity {
    private static final String TAG = ArMeasureActivityAlt.class.getSimpleName();
    ...
    private FloatingActionButton conf;
    String result;
    double distanceCm;
    Double  resultado;
    Peixes peixes;
...
//                popUp.showAsDropDown(v, 0, 0); // show popup like dropdown list
        ...
        peixes = (Peixes) getIntent().getSerializableExtra(Peixes.TAG_PEIXE);

        conf.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v) {
                if(anchors.size() >=2){
                   if(getResultado() >= peixes.getMin() && getResultado() <= peixes.getMax()){
                       Intent intent = new Intent(ArMeasureActivityAlt.this, MedidaPermitida.class);
                       peixes.setMedida(getResultado());
                       intent.putExtra(Peixes.TAG_PEIXE, peixes);
                       intent.setClass(ArMeasureActivityAlt.this, MedidaPermitida.class);
                       startActivity(intent);
                   }
                   else {
                       Intent intent = new Intent(ArMeasureActivityAlt.this, MedidaNegada.class);
                       peixes.setMedida(getResultado());
                       intent.putExtra(Peixes.TAG_PEIXE, peixes);
                       intent.setClass(ArMeasureActivityAlt.this, MedidaNegada.class);
                       startActivity(intent);
                   }
                }
                else {}
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                messageSnackbar = Snackbar.make(
                        ArMeasureActivityAlt.this.findViewById(android.R.id.content),
                        "Procurando por uma superficie...", Snackbar.LENGTH_INDEFINITE);
...
//            log(TAG, "onDrawFrame(), mTouches.size=" + mTouches.size());
            ...
            try {
                ...
                ArMeasureActivityAlt.this.pointCloud.update(pointCloud);
                ArMeasureActivityAlt.this.pointCloud.draw(viewmtx, projmtx);
                ...
                if(anchors.size() < 1){
                    ...
                    for(int i = 1; i < anchors.size(); i++){
                        ...
                        distanceCm = ((int)(getDistance(point0, point1) * 1000))/10.0f;
                        total += distanceCm;
                    ...
                    // show result
                    resultado = Double.valueOf((((int) (total * 10f)) / 10f));
                    result =  (((int) (total * 10f)) / 10f) + "cm";
                    showResult(result);
                ...
                if(tap != null && camera.getTrackingState() == TrackingState.TRACKING){
                    for (HitResult hit : frame.hitTest(tap)) {
                        ...
                        if (trackable instanceof Plane && ((Plane) trackable).isPoseInPolygon(hit.getHitPose())
                            ...
                            if (anchors.size() >= LIMIT_CUBE_COUNT) {
